...
import { Wifi, PlusCircle, Eye, EyeOff, ShieldCheck, ArrowUpRight, LogIn } from "lucide-react";
...
export const TransportCard: React.FC<TransportCardProps> = ({
  ...
  const isUnlinked = card.status === "unregistered" || card.id === "card_unlinked";
  const isFrozen = card.status === "frozen";

  const formattedNumber = isUnlinked
    ? "•••• •••• •••• ••••"
    : showFullNumber
    ...
    <div className="w-full">
      ...
      <div
        className={`relative overflow-hidden rounded-3xl p-6 sm:p-7 text-white shadow-cardGlow transition-all duration-300 ${
          isUnlinked
            ? "bg-gradient-to-br from-slate-800 via-slate-850 to-slate-900 border border-slate-700/80"
            : isFrozen
      ...
      >
        ...
        <div className="absolute -left-12 -bottom-12 w-64 h-64 rounded-full bg-[#00A3E0]/20 blur-2xl pointer-events-none" />
        ...
        <div className="relative z-10 flex items-start justify-between mb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="block text-[10px] text-cyan-100 font-medium">
                {isUnlinked ? "Transport Wallet Pass" : "Public Transport Pass"}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              />
              {isUnlinked ? "Logged Out" : isFrozen ? "Card Frozen" : "Active"}
            </span>
        ...
        {/* EMV Microchip Graphic & AC Mobility Speed Lines */}
        <div className="relative z-10 flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-11 h-8 rounded-md bg-gradient-to-tr from-amber-300 via-amber-200 to-amber-400 border border-amber-500/40 p-1 flex flex-col justify-between shadow-xs opacity-80">
        ...
        <div className="relative z-10 mb-5">
          <span className="text-[11px] font-semibold text-cyan-100 uppercase tracking-wider block">
            {isUnlinked ? "Card Balance" : "Available Balance"}
          </span>
          ...
          {isUnlinked && (
            <span className="text-xs text-slate-300 mt-1 block">
              Sign in to view your live card balance
            </span>
          )}
        </div>

        {/* Card Number & AC Mobility Footer */}
            ...
            <div className="grid grid-cols-2 gap-3">
              <Link href="/auth/login" className="w-full">
                <Button
                  ...
                  leftIcon={<LogIn className="w-5 h-5 text-white" />}
                >
                  Sign In
                </Button>
              </Link>
